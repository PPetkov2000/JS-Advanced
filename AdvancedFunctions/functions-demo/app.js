...
(function (data, document) {
  ...
  const fieldsMap = {
    ...
    friends: (_, list) =>
      renderUl(list.map((f) => renderLi(`${f.first_name} ${f.last_name}`))),
  ...
  const headingsMap = [
    ...
    a[b] = (type, content) =>
      `<a class="filter" data-sortby="${type}">${content}</a>`;
    return a;
  }, {});

  const defaultTd = chooseContentType.bind(undefined, fieldsMap, renderTd);

  const defaultTh = chooseContentType.bind(undefined, headingsMap, renderTh);
  ...
  function main(data) {
    return renderTable(
      renderThead(renderTr(keys.map((key) => defaultTh(key, dict[key])))) +
        renderTbody(
          data.map((row) =>
            renderTr(keys.map((cell) => defaultTd(cell, row[cell])))
          )
        )
    );
  }
